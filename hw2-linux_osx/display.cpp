...
void display() 
{
  ...
  if (numused) {
    ...
    for (int i = 0; i < numused; i++){
      GLfloat lightposT[4] = { lightposn[i*4], lightposn[(i*4)+1], lightposn[(i*4)+2], lightposn[(i*4)+3]}; 

      GLfloat out[4];
      transformvec(lightposT, out);

      lightransf[(i*4)] = out[0];
      lightransf[(i*4)+1] = out[1];

      lightransf[(i*4)+2] = out[2];
      
      lightransf[(i*4)+3] = out[3];
...
uniform float shininess; 
    ...
    const GLfloat shinNum[] = {obj->shininess};
    glUniform1fv(shininesscol, 1, shinNum);
