...
Intersec* intersecFind(vec3 P0, vec3 direction, bool setN, bool stopFir, float maxDist) {
    ...
    for (int i = 0; i < triangles.size(); i++) {
        ...
        if (r->isIntersected && r->t > 0 && r->t < minDist && r->t < maxDist) {
            ...
            if (isIntersected) delete isIntersected;
            isIntersected = new Intersec;
            isIntersected->t = minDist;
            isIntersected->obj = hitObject;
            isIntersected->isIntersected = true;
            isIntersected->p_intersect = r->p_intersect;
            if (setN) isIntersected->normal = isIntersected->obj->getNormal(isIntersected->p_intersect);
            found = true;
            if (stopFir) break;
        }
        if (r) delete r;
...
vec3 getSpecularDir(vec3 rayDir, vec3 norm){
	return glm::normalize(2 * glm::dot(-rayDir, norm) * norm + rayDir);
}
...
vec3 colorize(Intersec *r, int depth, vec3 pointOfView){
	...
	if (depth){
		...
		if (newIntersection && newIntersection->isIntersected && (newIntersection->t > 0)){
...
int main(int argc , char* argv[]){
	...
	if (argc < 2){
		...
		//FIBITMAP *img = FreeImage_ConvertFromRawBits(pixels, imgWidth, imgHeight,imgWidth*3, 24, 0x0000FF,0x00FF00, 0xFF0000, true);
		// FreeImage_Save(FIF_JPEG, img, outputFileName, 0);
